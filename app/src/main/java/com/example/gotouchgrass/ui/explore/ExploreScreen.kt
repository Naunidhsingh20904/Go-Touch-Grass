...
fun ExploreScreen(viewModel: ExploreViewModel) {
    ...
    LaunchedEffect(viewModel.pendingChallengeSnackbarMessage) {
...
fun ChallengeCard(
    ...
    val progressValue = if (card.isCompleted) 1f else card.progressFraction.coerceIn(0f, 1f)
    val primaryTextColor = if (card.isCompleted) {
        MaterialTheme.colorScheme.onSurfaceVariant
    } else {
        MaterialTheme.colorScheme.onSurface
    }
    val secondaryTextColor = if (card.isCompleted) {
        ...
        Row(
            ...
            Column(
                ...
                Row(
                    ...
                    Spacer(Modifier.width(GoTouchGrassDimens.SpacingSm))
            ...
            Column(
                ...
                Text(
                    text = if (card.isCompleted) "Completed" else "+${card.reward} XP",
                    style = MaterialTheme.typography.labelLarge,
                    color = if (card.isCompleted) {
                        MaterialTheme.colorScheme.onSurfaceVariant
                    } else {
    ...
    Box(
        ...
        ElevatedCard(
            ...
            Column(
                ...
                Text(
                    "Estimated time: ${route.hours} hours",
                    style = MaterialTheme.typography.bodySmall
                )
